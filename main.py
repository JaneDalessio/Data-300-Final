...
# ── Import all three model modules ───────────────────────────────────────────────
from eda               import run_eda
from jane_models       import run_jane_models
from aidan_models      import run_aidan_models
from katherine_models  import run_katherine_models
...
def main():
    ...
    # ════════════════════════════════════════════════════════════════════════════
    # 1. RUN EDA (once — all models share this output)
    # ════════════════════════════════════════════════════════════════════════════
    ...
    # ════════════════════════════════════════════════════════════════════════════
    # 2. RUN ALL THREE MODEL FILES
    # ════════════════════════════════════════════════════════════════════════════
    jane_results      = run_jane_models(X_train, X_test, y_train, y_test, tscv, feature_names)
    aidan_results     = run_aidan_models(X_train, X_test, y_train, y_test, tscv, feature_names)
    katherine_results = run_katherine_models(X_train, X_test, y_train, y_test, tscv, feature_names)
    ...
    # ════════════════════════════════════════════════════════════════════════════
    # 4. COMPARISON BAR CHART (RMSE, MAE, R²)
    # ════════════════════════════════════════════════════════════════════════════
    ...
    legend_elements = [
        Patch(facecolor="#1A3A6E", label="Jane (OLS, KNN)"),
        Patch(facecolor="#6A4BA5", label="Aidan (Ridge, SGD)"),
        Patch(facecolor="#0A7E8C", label="Katherine (Lasso, Elastic Net)"),
    ]
    ...
    # ════════════════════════════════════════════════════════════════════════════
    # 5. MONTHLY RMSE BREAKDOWN — all 6 models on the same axes
    ...
    _plot_all_monthly_rmse(all_results, X_train, X_test, y_train, y_test,
                            tscv, feature_names, comparison)

    # ════════════════════════════════════════════════════════════════════════════
    # 6. RMSE HEATMAP — model × month
    #    Compact visual for poster / report
    # ════════════════════════════════════════════════════════════════════════════
    _plot_rmse_heatmap(all_results, X_train, X_test, y_train, y_test,
                       tscv, feature_names, comparison)
...
def _plot_all_monthly_rmse(all_results, X_train, X_test, y_train, y_test,
                            tscv, feature_names, comparison):
...
def _plot_rmse_heatmap(all_results, X_train, X_test, y_train, y_test,
                        tscv, feature_names, comparison):
    fitted  = _refit_all_models(X_train, y_train, tscv)
    X_plot  = X_test.copy()
    X_plot["Actual"] = y_test.values
